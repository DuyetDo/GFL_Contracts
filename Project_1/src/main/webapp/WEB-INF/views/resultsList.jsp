...
<head>
    ...
    <title>Results List | Global Flow JSC</title>
    <!-- GFL icon-->
	...
	<section class="content">
		<div class="container-fluid">
			<div class="block-header">
				<h2>DANH SÁCH CÁC DỰ ÁN ĐÃ ĐƯỢC ĐÁNH GIÁ</h2>
			</div>
			<!-- Edit Table -->
			<!-- #End# Edit Table -->
			<!-- Exportable Table -->
					...
					<div class="card">
						...
						<div class="body">
							<table
								...
								<tbody>
									<c:forEach items="${results}" var="it">
										<c:set var="laiGop" value="${it.laiGop}"></c:set>
										<c:set var="lnr" value="${it.lnRong}"></c:set>
										<c:set var="tllg" value="${it.tlLaiGop}"></c:set>
										<c:set var="lnr_gv" value="${it.lnrTrenGv}"></c:set>
										<c:set var="lnr_gt" value="${it.lnrTrenValue}"></c:set>
										<tr>
											<td>${it.project.name}</td>
											<td><fmt:setLocale value="vi_VN" scope="session" /> <fmt:formatNumber
													value="${laiGop}" type="currency" /></td>
